config.simulations <- function() {
  ...
  ######  --------------------------------------------------------------------------
  {
    ...
    change.list <- data.frame(
      "tc.max" = climate.change.scenarios$max_temperature_change_degC, # {e.g., 0, 1, 2, ...} (changes in temperature)
  ...
  ######  -----------------------------------------------------------------------------
  {
    ...
    start.date.synoptic <- "1948-01-01"
    end.date.synoptic <- "2021-12-31" # from processed GPHA file

    start.date.WRs <- "1948-01-01"
    end.date.WRs <- "2019-12-31" # proper leap year orders (starting with leap year of 1948, ending a year before (i.e., 2019) the leap year of 2020)
