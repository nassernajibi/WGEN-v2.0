...
WR_prob_estimator <- function(WR_prob_chunk, WR_prob_change,
  ...
  C <- c(
    rep(0, num_periods), # no cost on probabilities for different time periods
  ...
  output <- list(optimum$status, 
                 sampling_probabilities,
                 deviation_from_WR_target, 
                 deviation_from_prob_unif, 
                 deviation_from_prob_unif_piecewise, 
                 optimum$objval)
