wgen.simulator <- function(weather.state.assignments, mc.sim, prcp.basin,
                           dates.weather, first.month, last.month, dates.sim,
  ...
  clust.run.obs <- cbind(clust.run.obs, 
                         "lag1.prcp" = lag1.prcp.basin.obs, 
                         "lag1.prcp.occur" = lag1.prcp.basin.obs.occur, 
                         "missing" = num.missing.prcp.basin, 
                         "basin.avg" = avg.prcp.basin)
  #################################
  ...
  for (i in 1:num.clust.run.sim) {
    ...
    if ((clust.run.sim$first.run[i] - 1) <= 0) {
      lag1.prcp.basin <- 0
      lag1.prcp.basin.occur <- 0
    } # for day one of simulation
    if (lag1.prcp.basin > min.thresh) {
      lag1.prcp.basin.occur <- 1
    }
