execute.WRs.non_param.NHMM <- function() {
  ...
  # Pre-allocate lists with `n.seasons` length
  covariates.final <- vector("list", n.seasons)
  dates.final <- vector("list", n.seasons)
  hgt.final <- vector("list", n.seasons)
  ...
  fit.mod.NHMM <- vector("list", n.seasons) # final NHMMs fit by season

  for (s in 1:n.seasons) {
    
    cat('\n NHMM fitting for season: ', s, '\n')
    ...
    # Define the covariate matrix and the formula (fo) of the multinomial regression in the NHMM
  ...
  # Glue different seasons together  
  ...
  WR.historical.s <- vector("list", n.seasons)
  ...
  for (s in 1:n.seasons) {
    ...
    if (s > 1) {
      to.add <- sum(num_WRs.season[1:(s - 1)])
    } else {
      to.add <- 0
    } # relabel the WRs, based on WRs from previous seasons
  ...
  output <- WR_prob_estimator(
    WR_prob_chunk, WR_prob_change, lp.threshold, piecewise_limit,
  ...
  set.seed(n.segment)
  my.replication <- sample(seq(1, num.segment), 
                           total.seqments,
                           replace = TRUE,
                           prob = my.prob
  )
  ...
  final.NHMM.output <- list(dates.historical = dates.historical,
                            WR.historical = weather.state.assignments,
                            dates.sim = dates.sim,
                            WR.simulation = markov.chain.sim,
